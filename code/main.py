...
tf.app.flags.DEFINE_string("debug", "false", "train on tiny data set")
tf.app.flags.DEFINE_string("model", "baseline", "by default donot enable BiDAF model")
tf.app.flags.DEFINE_string("is_training", "yes", "for dropout in transformernetwork")
tf.app.flags.DEFINE_string("seek_attn", "false", "write file contains attn")
...
def main(unused_argv):
    ...
    with open('id2word.txt', 'w') as f:
        f.write(json.dumps(id2word, ensure_ascii=False))


    # Get filepaths to train/dev datafiles for tokenized queries, contexts and answers
    if FLAGS.debug == "true":
        train_context_path = os.path.join(FLAGS.data_dir, "tiny-train.context")
        train_qn_path = os.path.join(FLAGS.data_dir, "tiny-train.question")
        train_ans_path = os.path.join(FLAGS.data_dir, "tiny-train.span")
    else:
    ...
    if FLAGS.mode == "train":
        ...
        with tf.Session(config=config) as sess:
            ...
            answers_dict = generate_answers(sess, qa_model, word2id, qn_uuid_data, context_token_data, qn_token_data, seek_attn=FLAGS.seek_attn)
