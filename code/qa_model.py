...
from modules import RNNEncoder, SimpleSoftmaxLayer, BasicAttn, BasicAttnPlusOne, BasicAttnPlusTwo,GatedAttn,SelfAttn,Output_Rnet,BiDAF, BiCoattn, TransformerNetwork, BCTN
...
class QAModel(object):
    ...
    def __init__(self, FLAGS, id2word, word2id, emb_matrix):
        ...
        if self.FLAGS.model == 'bidaf':
            print('Current Model: bidaf')
        elif self.FLAGS.model == 'bicoattn':
            print('Current Model: bicoattn')
        elif self.FLAGS.model == 'basicattnplusone':
            print('Current Model: basicattnplusone')
        elif self.FLAGS.model == 'basicattnplustwo':
            print('Current Model: basicattnplustwo')
        elif self.FLAGS.model == 'transformernetwork':
            print('Current Model: transformernetwork')
        elif self.FLAGS.model == 'rnet':
            print('Current Model: rnet')
        elif self.FLAGS.model == 'bctn':
            print('Current Model: bctn')
        else:
            print('Current Model: baseline')
        print('Model Config:')
        print('learning rate : %s' % (self.FLAGS.learning_rate))
        print('dropout: %s' % (self.FLAGS.dropout))
        print('batch_size: %s' % (self.FLAGS.batch_size))
        print('hidden_size: %s' % (self.FLAGS.hidden_size))
        print('context_len: %s' % (self.FLAGS.context_len))
        print('question_len: %s' % (self.FLAGS.question_len))
        ...
        # if (self.FLAGS.model == 'bidaf') | (self.FLAGS.model == 'bicoattn'):
            # opt = tf.train.AdadeltaOptimizer(learning_rate=self.FLAGS.learning_rate, rho=0.999) # you can try other optimizers
        if self.FLAGS.model == 'bctn':
            # lr = self.FLAGS.learning_rate * tf.minimum(tf.pow(tf.cast(self.global_step, 'float'), tf.cast(-0.5, 'float')), tf.multiply(tf.cast(self.global_step, 'float'), tf.cast(400 ** (-1.5), 'float')))
            # print('bctn lr: {}'.format(lr))
            opt = tf.train.AdamOptimizer(learning_rate=self.FLAGS.learning_rate, beta1=0.9, beta2=0.98, epsilon=1e-9)
        else:
    ...
    def build_graph(self):
        ...
        if self.FLAGS.model == 'bidaf':
            bidaf_layer = BiDAF(self.FLAGS.hidden_size, self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
            g_m, g_m2 = bidaf_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # (batch_size, context_len, hidden_size*10)
            with vs.variable_scope("StartDist"):
                softmax_layer_start = SimpleSoftmaxLayer(1 - self.FLAGS.dropout)
                self.logits_start, self.probdist_start = softmax_layer_start.build_graph(g_m, self.context_mask)
            with vs.variable_scope("EndDist"):
                softmax_layer_end = SimpleSoftmaxLayer(1 - self.FLAGS.dropout)
                self.logits_end, self.probdist_end = softmax_layer_end.build_graph(g_m2, self.context_mask)
        elif self.FLAGS.model == 'bicoattn':
            bicoattn_layer = BiCoattn(self.FLAGS.batch_size, self.FLAGS.context_len, self.FLAGS.hidden_size, self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
            g_m, g_m2, self.attn_output = bicoattn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # (batch_size, context_len, hidden_size*14)
            with vs.variable_scope("StartDist"):
                softmax_layer_start = SimpleSoftmaxLayer(1 - self.FLAGS.dropout)
                self.logits_start, self.probdist_start = softmax_layer_start.build_graph(g_m, self.context_mask)
            with vs.variable_scope("EndDist"):
            ...
            transformernetwork_layer = TransformerNetwork(self.FLAGS.hidden_size, self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2, 3, 8)
            g_m, g_m2 = transformernetwork_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask, self.FLAGS.is_training) # (batch_size, context_len, hidden_size*2)
            with vs.variable_scope("StartDist"):
                softmax_layer_start = SimpleSoftmaxLayer(1 - self.FLAGS.dropout)
                self.logits_start, self.probdist_start = softmax_layer_start.build_graph(g_m, self.context_mask)
            with vs.variable_scope("EndDist"):
            ...
            bctn_layer = BCTN(self.FLAGS.hidden_size, self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2, 2, 8)
            g_m, g_m2 = bctn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask, self.FLAGS.is_training) # (batch_size, context_len, hidden_size*2)
            with vs.variable_scope("StartDist"):
                softmax_layer_start = SimpleSoftmaxLayer(1 - self.FLAGS.dropout)
                self.logits_start, self.probdist_start = softmax_layer_start.build_graph(g_m, self.context_mask)
            with vs.variable_scope("EndDist"):
            ...
            with vs.variable_scope("SelfAttn"): 
                ...
                attn_output_self, self.attn_output = attn_layer_self.build_graph(context_hiddens_gated, self.context_mask) # (batch_size, context_len, hidden_size * 2)

            print "Output"
            with vs.variable_scope("Output"): 
                output_layer = Output_Rnet(self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size)
            ...
            attn_layer = BasicAttnPlusOne(self.FLAGS.batch_size, self.FLAGS.context_len, self.FLAGS.hidden_size, self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
            _, attn_output = attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # attn_output is shape (batch_size, context_len, hidden_size*4)
            ...
            attn_layer = BasicAttnPlusTwo(self.FLAGS.batch_size, self.FLAGS.context_len, self.FLAGS.hidden_size, self.keep_prob, self.FLAGS.hidden_size*2, self.FLAGS.hidden_size*2)
            g_m, g_m2 = attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens, self.context_mask) # attn_output is shape (batch_size, context_len, hidden_size*4)
            with vs.variable_scope("StartDist"):
                softmax_layer_start = SimpleSoftmaxLayer(1 - self.FLAGS.dropout)
                self.logits_start, self.probdist_start = softmax_layer_start.build_graph(g_m, self.context_mask)
            with vs.variable_scope("EndDist"):
            ...
            _, attn_output = attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens) # attn_output is shape (batch_size, context_len, hidden_size*4)
    ...
    def run_train_iter(self, session, batch, summary_writer):
        ...
        # logging.debug('a galance on batch.context_ids: %s', batch.context_ids)
        # logging.debug('a galance on batch.context_mask: %s', batch.context_mask)
        # logging.debug('a galance on batch.qn_ids: %s', batch.qn_ids)
        # logging.debug('a galance on batch.qn_mask: %s', batch.qn_mask)
        # logging.debug('a galance on batch.ans_span: %s', batch.ans_span)
    ...
    def get_prob_dists(self, session, batch):
        ...
        input_feed[self.qn_mask] = batch.qn_mask
        # note you don't supply keep_prob here, so it will default to 1 i.e. no dropout
        if self.FLAGS.seek_attn != 'true':
            output_feed = [self.probdist_start, self.probdist_end]
            [probdist_start, probdist_end] = session.run(output_feed, input_feed)
            return probdist_start, probdist_end
        else:
            output_feed = [self.probdist_start, self.probdist_end, self.attn_output]
            [probdist_start, probdist_end, self.attn_output_] = session.run(output_feed, input_feed)
            return probdist_start, probdist_end
    ...
    def get_start_end_pos(self, session, batch):
        ...
        self.start_dist_, self.end_dist_ = start_dist, end_dist
