...
def generate_answers(session, model, word2id, qn_uuid_data, context_token_data, qn_token_data, seek_attn='false'):
    """
    ...
    if seek_attn != 'true':
        ...
        print('Init attn file !')
        attn_file = open('attn_file.txt', 'w')
    ...
    for batch in get_batch_generator(word2id, qn_uuid_data, context_token_data, qn_token_data, model.FLAGS.batch_size, model.FLAGS.context_len, model.FLAGS.question_len):
        ...
        if seek_attn != 'true':
            ...
            print('Writing attn file !')
            temp_dict = {'context_ids': batch.context_ids.tolist(), 'qn_ids': batch.qn_ids.tolist(), \
            'attn_output': model.attn_output_.tolist(), 'start_dist': model.start_dist_.tolist(), 'end_dist': model.end_dist_.tolist(), \
            'pred_start_batch': pred_start_batch, 'pred_end_batch': pred_end_batch}
            attn_file.write(json.dumps(temp_dict, ensure_ascii=False))
            attn_file.write('\n')
